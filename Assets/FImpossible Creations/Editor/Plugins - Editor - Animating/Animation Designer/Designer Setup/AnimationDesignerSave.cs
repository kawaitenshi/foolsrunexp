...
namespace FIMSpace.AnimationTools
{
    public partial class AnimationDesignerSave : ScriptableObject
    {
        ...
        /// <summary> Different settings setups for different animation clips </summary>
        public List<ADClipSettings_Morphing> BlendingSetupsForBlendings = new List<ADClipSettings_Morphing>();
        ...
        internal void CopySettingsFromTo(AnimationClip clp, int from, int to, AnimationDesignerSave save)
        {

            ADClipSettings_Main preM = GetSetupForClip(MainSetupsForClips, clp, from);

            for (int i = 0; i < MainSetupsForClips.Count; i++)
                if (MainSetupsForClips[i].SetIDHash == to) { MainSetupsForClips[i] = preM.Copy(MainSetupsForClips[i]); break; }

            ADClipSettings_IK preIK = GetSetupForClip(IKSetupsForClips, clp, from);
            for (int i = 0; i < IKSetupsForClips.Count; i++)
                if (IKSetupsForClips[i].SetIDHash == to) { IKSetupsForClips[i] = preIK.Copy(IKSetupsForClips[i]); break; }

            ADClipSettings_Elasticness preEl = GetSetupForClip(ElasticnessSetupsForClips, clp, from);
            for (int i = 0; i < ElasticnessSetupsForClips.Count; i++)
                if (ElasticnessSetupsForClips[i].SetIDHash == to) { ElasticnessSetupsForClips[i] = preEl.Copy(ElasticnessSetupsForClips[i]); break; }


            ADClipSettings_Modificators preMods = GetSetupForClip(ModificatorsSetupsForClips, clp, from);

            for (int i = 0; i < ModificatorsSetupsForClips.Count; i++)
                if (ModificatorsSetupsForClips[i].SetIDHash == to) { ModificatorsSetupsForClips[i] = preMods.Copy(ModificatorsSetupsForClips[i], save); break; }
