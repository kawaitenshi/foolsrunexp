    ...
    public partial class RagdollProcessor
    {
        ...
        public static void Editor_DrawTweakGUI(SerializedProperty sp_param, RagdollProcessor proc)
        {
            ...
            if (proc.lminArmsAnim > 0f || proc.minRArmsAnim > 0f || proc.minSpineAnim > 0f || proc.minHeadAnim > 0f || proc.minChestAnim > 0f)
            {
                ...
                if (proc.lminArmsAnim > 0.0001f) EditorGUILayout.Slider("Left Arm Blend In", proc.lminArmsAnim, 0f, 1f);
                if (proc.minRArmsAnim > 0.0001f) EditorGUILayout.Slider("Right Arm Blend In", proc.minRArmsAnim, 0f, 1f);
                if (proc.minHeadAnim > 0.0001f) EditorGUILayout.Slider("Head Blend In", proc.minHeadAnim, 0f, 1f);
                if (proc.minChestAnim > 0.0001f) EditorGUILayout.Slider("Chest Blend In", proc.minChestAnim, 0f, 1f);
                if (proc.minSpineAnim > 0.0001f) EditorGUILayout.Slider("Spine Blend In", proc.minSpineAnim, 0f, 1f);
